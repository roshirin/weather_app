<template>
  <div class="weather-card">
    <div class="current-info">
      <div class="current-info__city">{{ selectedCity }}</div>
      <div class="current-info__description">
        <div class="current-info__temperature">{{ cardData[0].temperature }}°C</div>
        <img :src="getIconPath(cardData[0].iconSrc)" class="current-info__icon" />
      </div>
    </div>
    <div v-if="cardData.length > 1" class="current-info">
...
<script>
  export default {
    ...
    methods: {
      getIconPath(iconSrc) {
        return require(`@/img/icons/${iconSrc}`);
      },
    },
  };
</script>
